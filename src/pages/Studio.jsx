...
/* ── Real pricing from business config ── */
const FLOOR_TYPES = CONFIG.pricing.floorTypes
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className="flex justify-center mb-5">
        <div className="inline-flex bg-warm-100 rounded-xl p-1 gap-1">
          {['visualizer', 'calculator'].map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-6 py-2.5 rounded-lg text-sm font-semibold transition-all cursor-pointer ${tab === t ? 'bg-white text-warm-800 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}>
...
/* ══════════════════════════════════════════
   VISUALIZER — one box, everything in it
   ══════════════════════════════════════════ */
function VisualizerTab() {
  ...
  const handleRoomFile = (file) => {
    if (!file?.type.startsWith('image/')) return
    setRoomFile(file)
    const r = new FileReader()
    r.onload = (e) => setRoomImage(e.target.result)
    r.readAsDataURL(file)
    setResultImage(null); setError(null)
  }

  const handleSampleFile = (file) => {
    if (!file?.type.startsWith('image/')) return
    setSampleFile(file)
    const r = new FileReader()
    r.onload = (e) => setSampleImage(e.target.result)
    r.readAsDataURL(file)
    setPrompt(''); setResultImage(null); setError(null)
  }
  ...
  const reset = () => {
    setRoomImage(null); setRoomFile(null); setSampleImage(null); setSampleFile(null)
    setPrompt(''); setResultImage(null); setError(null)
  }
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-warm-100 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-warm-100">
          <div className="flex items-center gap-2">
            <a href={CONFIG.cta.quote.href} className="inline-flex items-center gap-1.5 bg-warm-700 hover:bg-warm-800 text-white px-5 py-2.5 rounded-lg font-semibold text-xs transition-all no-underline active:scale-[0.98]">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
              </svg>
              {CONFIG.cta.quote.text}
            </a>
            <button onClick={() => { const a = document.createElement('a'); a.download = 'my-new-floor.jpg'; a.href = resultImage; a.click() }}
    ...
    <div>
      ...
      <div className="bg-warm-50 border border-warm-200 rounded-xl px-5 py-3.5 mb-4 flex items-start gap-3">
        ...
        <div className="text-sm text-stone-600 leading-relaxed">
          ...
          <span className="text-stone-400 ml-1">Upload your room, pick a floor, and hit Visualize. Not sure what type? See our <a href="/resources" className="text-warm-600 hover:text-warm-700 underline">flooring guide</a>.</span>
        </div>
      </div>

    <div className="bg-white rounded-2xl shadow-sm border border-warm-100 overflow-hidden">
      {/* Top: room photo + sample upload side by side */}
      <div className="flex">
        {/* Room photo — takes most of the space */}
        <div className="flex-1 min-w-0">
          {!roomImage ? (
            <div
              ...
              onDrop={(e) => { e.preventDefault(); handleRoomFile(e.dataTransfer.files?.[0]) }}
            >
              ...
              <input ref={roomRef} type="file" accept="image/*" capture="environment" className="hidden"
                onChange={(e) => handleRoomFile(e.target.files?.[0])} />
            </div>
          ) : (
            <div className="relative h-[360px]">
              <img src={roomImage} alt="Your room" className="w-full h-full object-cover" />
              <button onClick={() => { setRoomImage(null); setRoomFile(null); if (roomRef.current) roomRef.current.value = '' }}
                className="absolute top-3 right-3 bg-black/50 hover:bg-black/70 text-white w-7 h-7 rounded-full flex items-center justify-center cursor-pointer text-xs backdrop-blur-sm">✕</button>
            </div>
          )}
        </div>

        {/* Sample upload — narrow column */}
        <div className="w-44 shrink-0 border-l border-warm-100 flex flex-col">
          ...
          {!sampleImage ? (
            ...
            >
              ...
              <input ref={sampleRef} type="file" accept="image/*" className="hidden"
                onChange={(e) => handleSampleFile(e.target.files?.[0])} />
            </div>
          ) : (
            <div className="flex-1 relative">
              <img src={sampleImage} alt="Sample" className="w-full h-full object-cover" />
              <button onClick={() => { setSampleImage(null); setSampleFile(null); if (sampleRef.current) sampleRef.current.value = '' }}
                className="absolute top-2 right-2 bg-black/50 hover:bg-black/70 text-white w-6 h-6 rounded-full flex items-center justify-center cursor-pointer text-[10px] backdrop-blur-sm">✕</button>
            </div>
          )}
        </div>
      </div>

      {/* Bottom: prompt bar + generate */}
      <div className="px-4 py-3 border-t border-warm-100 bg-stone-50/50">
        ...
        <div className="relative mb-2.5">
          <input
            type="text"
            value={prompt}
            onChange={(e) => { setPrompt(e.target.value); if (sampleFile) { setSampleFile(null); setSampleImage(null) } }}
            placeholder={sampleFile ? 'Using uploaded sample — or type here to describe instead' : 'Describe the flooring you want — e.g. "dark walnut hardwood planks"'}
            ...
            {suggestions.map((s) => (
              <button key={s} onClick={() => setPrompt(s)}
                className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-all cursor-pointer ${
                  prompt === s ? 'bg-warm-700 text-white' : 'bg-white text-stone-500 border border-warm-200 hover:border-warm-400 hover:text-stone-700'
                }`}>
                {s}
              </button>
            ))}
        ...
        {error && (
          <div className="mt-2.5 bg-red-50 border border-red-200 rounded-lg px-3 py-2 flex items-center justify-between">
            ...
            <button onClick={() => setError(null)} className="text-red-400 hover:text-red-600 text-xs cursor-pointer ml-3">Dismiss</button>
          </div>
        )}
      </div>
    </div>
...
/* ══════════════════════════════════════════
   COST CALCULATOR — real pricing, ranges
   ══════════════════════════════════════════ */
function CalculatorTab() {
  ...
  const [installType, setInstallType] = useState('pro')
  ...
  const [tilePattern, setTilePattern] = useState(false) // herringbone etc
  ...
  const sqft = useMemo(() => {
    ...
    const w = parseFloat(width); const l = parseFloat(length)
  ...
  const estimate = useMemo(() => {
    ...
    // materials: base + per-type extras
    let materialPerSqft = floor.material
    for (const v of Object.values(floor.perSqft)) materialPerSqft += v
    ...
    if (isPro) {
      ...
      // baseboard: material always, labor pro only
      ...
      // DIY still pays for transition pieces and baseboard material
      extrasCost += doorways * EXTRAS_PRICING.doorwayTransition
    ...
    // totals
    const tax = (materialsCost + (baseboardLf * EXTRAS_PRICING.baseboard.material) + (quarterRoundLf * EXTRAS_PRICING.quarterRound.material) + (doorways * EXTRAS_PRICING.doorwayTransition)) * TAX_RATE

    const subtotal = materialsCost + laborCost + prepCost + extrasCost
    const withMargin = subtotal * (1 + MARGIN)
    const total = withMargin + tax

    // range: low is our price, high is +15%
    const low = total
  ...
  useEffect(() => {
    ...
    const startLow = displayLow; const startHigh = displayHigh
    let frame = 0
  ...
  }, [estimate?.low, estimate?.high, displayLow, displayHigh])
  ...
  const NumInput = ({ label, value, onChange, max = 20 }) => (
    <div className="flex items-center justify-between">
      <span className="text-xs text-stone-600">{label}</span>
      <div className="flex items-center gap-2">
        <button onClick={() => onChange(Math.max(0, value - 1))} className="w-9 h-9 rounded-lg bg-warm-100 hover:bg-warm-200 active:bg-warm-300 text-warm-800 text-lg font-bold flex items-center justify-center cursor-pointer transition-colors select-none">-</button>
        <span className="w-8 text-center text-sm font-bold text-stone-800">{value}</span>
        <button onClick={() => onChange(Math.min(max, value + 1))} className="w-9 h-9 rounded-lg bg-warm-100 hover:bg-warm-200 active:bg-warm-300 text-warm-800 text-lg font-bold flex items-center justify-center cursor-pointer transition-colors select-none">+</button>
      </div>
    </div>
  )
  ...
  const Toggle = ({ label, checked, onChange }) => (
    <div className="flex items-center justify-between">
      ...
      <button onClick={() => onChange(!checked)}
        ...
        <div className={`w-4 h-4 rounded-full bg-white shadow-sm absolute top-0.5 transition-all ${checked ? 'left-[18px]' : 'left-0.5'}`} />
      </button>
    </div>
  )
    ...
    <div>
      ...
      <div className="bg-warm-50 border border-warm-200 rounded-xl px-5 py-3.5 mb-5 flex items-start gap-3">
        ...
        <div className="text-sm text-stone-600 leading-relaxed">
          ...
          <span className="text-stone-400 ml-1">Need help measuring? Try our <a href="/resources" className="text-warm-600 hover:text-warm-700 underline">sq ft guide &amp; practice game</a>.</span>
        </div>
      </div>

    <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
      <div className="lg:col-span-3 space-y-4">
        {/* Row 1: Floor Type + Install Mode */}
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-warm-100">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex bg-stone-100 rounded-lg p-0.5 text-xs">
              <button onClick={() => setInstallType('pro')} className={`px-3 py-1.5 rounded-md font-medium transition-all cursor-pointer ${isPro ? 'bg-white text-warm-700 shadow-sm' : 'text-stone-500'}`}>Pro Install</button>
              <button onClick={() => setInstallType('diy')} className={`px-3 py-1.5 rounded-md font-medium transition-all cursor-pointer ${!isPro ? 'bg-white text-warm-700 shadow-sm' : 'text-stone-500'}`}>DIY</button>
            </div>
          </div>
          <div className="grid grid-cols-5 gap-1.5">
            {Object.entries(FLOOR_TYPES).map(([id, f]) => (
              <button key={id} onClick={() => { setFloorType(id); if (id !== 'tile') setTilePattern(false) }}
                ...
                <span className="text-[10px] font-semibold text-stone-700 text-center leading-tight">{f.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-warm-100">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex bg-stone-100 rounded-lg p-0.5 text-xs">
              <button onClick={() => setInputMode('dimensions')} className={`px-3 py-1.5 rounded-md font-medium transition-all cursor-pointer ${inputMode === 'dimensions' ? 'bg-white text-stone-800 shadow-sm' : 'text-stone-500'}`}>L x W</button>
              <button onClick={() => setInputMode('sqft')} className={`px-3 py-1.5 rounded-md font-medium transition-all cursor-pointer ${inputMode === 'sqft' ? 'bg-white text-stone-800 shadow-sm' : 'text-stone-500'}`}>Sq Ft</button>
            </div>
          </div>
          <div className="flex flex-wrap gap-1.5 mb-3">
            {presets.map((r) => (
              <button key={r.name} onClick={() => { setInputMode('dimensions'); setWidth(String(r.w)); setLength(String(r.l)) }}
          ...
          {inputMode === 'dimensions' ? (
            <div className="grid grid-cols-2 gap-3">
              {[['Length', length, setLength], ['Width', width, setWidth]].map(([label, val, setter]) => (
                <div key={label}>
                  <label className="block text-[10px] font-medium text-stone-500 mb-1 uppercase tracking-wide">{label} (ft)</label>
            ...
            <div>
              ...
              <input type="number" inputMode="decimal" value={totalSqft} onChange={(e) => setTotalSqft(e.target.value)} placeholder="e.g. 200"
                className="w-full px-3 py-2.5 rounded-xl border border-warm-200 focus:border-warm-500 focus:ring-2 focus:ring-warm-200/50 outline-none text-base font-semibold text-stone-800 bg-stone-50/50" />
            </div>
          )}
          {sqft > 0 && (
        ...
        {/* Row 3: Project Details — per-type questions */}
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-warm-100">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-warm-100">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between">
                  <span className="text-stone-500">Materials ({Math.ceil(estimate.coveredSqft)} sf incl. {Math.round(floor.waste * 100)}% waste)</span>
                ...
                <div className="flex justify-between pt-1.5 border-t border-stone-100">
                  <span className="text-stone-400">{CONFIG.pricing.taxName} {Math.round(TAX_RATE * 100)}% (materials)</span>
                ...
                <div className="flex justify-between pt-2 border-t-2 border-warm-200">
                  ...
                  <span className="font-bold text-warm-700">${fmt(estimate.low)} – ${fmt(estimate.high)}</span>
                </div>
                <div className="text-center text-[10px] text-stone-400 pt-1">
                  That's <strong className="text-stone-600">${(estimate.low / sqft).toFixed(2)} – ${(estimate.high / sqft).toFixed(2)}</strong> per sf {isPro ? 'installed' : 'materials'}
                </div>
              </div>
              ...
              <a href={CONFIG.cta.quote.href} className="mt-3 w-full bg-warm-700 hover:bg-warm-800 text-white py-3 rounded-xl font-semibold text-sm flex items-center justify-center no-underline transition-all active:scale-[0.98]">
                {CONFIG.cta.quote.text}
              </a>
            </div>
          )}
